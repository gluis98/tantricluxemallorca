...
class MasseuseCatalog
{
     ...
     * Si hay datos semilla (traducciones), los reutiliza por coincidencia de carpeta.
     */
    ...
    private static function matchSeed(string $dir, array $seedItems): ?array
    {
        foreach ($seedItems as $seed) {
            ...
            if ($image !== '' && str_contains($image, '/masseurs/' . $dir . '/')) {
                return $seed;
            }
            foreach (($seed['images'] ?? []) as $img) {
                if (is_string($img) && str_contains($img, '/masseurs/' . $dir . '/')) {
                    return $seed;
                }
            }
        }
